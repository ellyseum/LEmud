...
export class SnakeGameState implements ClientState {
  ...
  enter(client: ConnectedClient): void {
    ...
    client.stateData.previousState = client.stateData.previousState || ClientStateType.AUTHENTICATED;
  ...
  private drawGame(client: ConnectedClient): void {
    ...
    this.snake.forEach((segment, index) => {
      if (segment.x >= 0 && segment.x < this.boardWidth && 
          segment.y >= 0 && segment.y < this.boardHeight) {
    ...
    // Draw food
    if (this.food.x >= 0 && this.food.x < this.boardWidth && 
        this.food.y >= 0 && this.food.y < this.boardHeight) {
  ...
  private showHighScores(client: ConnectedClient): void {
    ...
    writeToClient(client, colorize(`${'Rank'.padEnd(5)}${'Player'.padEnd(usernameColWidth)}Score\r\n`, 'cyan'));
