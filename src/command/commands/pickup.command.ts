...
export class PickupCommand implements Command {
  ...
  private pickupItem(client: ConnectedClient, room: any, itemName: string): void {
    ...
    const itemNameDisplay = typeof item === 'string' ? item : item.name;
    
    // Remove the item from the room
    room.items.splice(itemIndex, 1);
    
    // Add the item to the player's inventory - store just the name for simplicity
    client.user.inventory.items.push(itemNameDisplay);
    ...
    // Notify the player
    writeToClient(client, colorize(`You pick up the ${itemNameDisplay}.\r\n`, 'green'));
  }
}
